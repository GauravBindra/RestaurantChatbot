...
def retrieve_relevant_chunks(query: str, top_k=5):
    """
    Retrieves relevant chunks from FAISS and applies Chain-of-Thought (CoT) reasoning before response generation.
    Ensures references are extracted and reasoning steps are logged.
    """
    filters_applied = extract_filters(query, csv_file_path)
    k = get_dynamic_k(query)

    # Step 1: Retrieve raw FAISS results
    results = vector_store.similarity_search(query, k=k)

    # Step 2: Apply strict filtering for restaurant-related queries
    is_restaurant_query = any(keyword in query.lower() for keyword in ["restaurant", "menu", "dish", "food", "serve"])
    ...
    if boosted_results:
        for i, doc in enumerate(boosted_results[:top_k]):  # Limit to top results
            source = doc.metadata.get("source", "unknown")

            # Extract references for structured restaurant data
            if source == "csv":
                record_id = doc.metadata.get("record_id", "N/A")
                references.append(f"📌 **Restaurant Database Record ID:** {record_id}")

            # Extract references for Wikipedia-based knowledge
            elif source == "wikipedia":
                title = doc.metadata.get("title", "Unknown")
                url = doc.metadata.get("url", "#")
                references.append(f"🔗 **Wikipedia Source:** [{title}]({url})")

            # Log reasoning steps
            metadata_info = ", ".join([f"{k}: {v}" for k, v in doc.metadata.items()])
            reasoning_steps.append(f"Step {i+1}: Found relevant data from **{source}** - {metadata_info}")

    else:
        reasoning_steps.append("Step 1: No exact match found. Expanding search to broader category.")

    return boosted_results, "\n".join(reasoning_steps), references
...
def generate_response(query: str, retrieved_chunks: list, reasoning_steps: str) -> str:
    """
    Generates a response using Mistral-7B-Instruct with structured and unstructured context, incorporating stepwise reasoning.
    """
    ...
    # System message to guide reasoning
    system_message = f"""
    ...
    - **Step 1:** Identify if the query is about a restaurant, menu item, pricing, or general food knowledge.
    - **Step 2:** Prioritize structured restaurant data for factual queries (CSV).
    - **Step 3:** Use Wikipedia data for general food history or trends.
    - **Step 4:** Ensure references are included if available.
    - **Step 5:** If no exact match exists, clearly state "I don't have enough information."
    ...
    response = llm(system_message, max_tokens=300, temperature=0.0)  # Reduce randomness for accuracy
...
# ------------------------- Modify Streamlit UI -------------------------
st.title("AI-Powered Restaurant & Food Knowledge Chatbot")
